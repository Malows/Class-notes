...
- [x] **Step 1: Inspect `schema.ts`**
  - Verify where table schemas and indices are initialized.

- [x] **Step 2: Append Unique Index definition**
...
- [x] **Step 1: Update `create` method in `PeriodRepositoryImpl`**
...
- [x] **Step 2: Update `update` method in `PeriodRepositoryImpl`**
  - Query if _another_ active period under the same subject possesses the target year and semester, excluding the current ID. If yes, throw `Error("Period already exists for this subject")`.
...
- [x] **Step 1: Update POST handler in `periods/+server.ts`**
...
- [x] **Step 2: Update PUT handler in `periods/[id]/+server.ts`**
...
- [x] **Step 1: Refactor `handleSave` in `+page.svelte`**
...
- [x] **Step 2: Update `PeriodModal.svelte` with serverError and ErrorSpan**
